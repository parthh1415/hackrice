...
python3 -m pytest tests/ -q              # 411 passing (3 skip: script labels that are keys, not buttons)
...
downstream of it is arithmetic. The network is touched only by `refresh_dataset.py`, which
you have to run on purpose.
...
```sh
python3 -m pytest tests/ -q      # 411, no network required
```
